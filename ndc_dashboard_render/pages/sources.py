...
def get_layout():
    return html.Div([
        ...
        html.Div(className="explainer-text", children=[
            ...
            dcc.Markdown("February 17, 2026"),

            html.H3("Count of countries with new NDCs", className="section-title"),
            dcc.Markdown("""
The total count of new NDCs is for all countries (not parties) that have submitted NDCs since January 1, 2024. This count includes the United States, even though the United States has withdrawn from the Paris Agreement and the Trump administration has signaled its intent to withdraw from the UNFCCC. (Data for the United States are not provided elsewhere in the database.) The count also includes the 27 member states of the EU (but not the EU itself as a party). Countries that are parties to the [UN Framework Convention on Climate Change](https://unfccc.int/process-and-meetings/united-nations-framework-convention-on-climate-change) but have not ratified the [Paris Agreement](https://unfccc.int/process-and-meetings/the-paris-agreement) are also included (e.g., Yemen, which has [submitted](https://unfccc.int/sites/default/files/2025-11/Yemens%20NDC3.0%20Vision.pdf) an NDC "vision and framework”). Some of the new NDCs submitted by countries since 2024 do not contain targets for 2035 but are nevertheless counted here as being new NDCs.
            """),

            html.H3("Past emissions trends", className="section-title"),
            dcc.Markdown("""
Data on past emissions are sourced primarily from parties' national inventory data reported to the UN since 2024, including in [Biennial Update Reports](https://unfccc.int/BURs) (BURs), [Biennial Transparency Reports](https://unfccc.int/first-biennial-transparency-reports) (BTRs), formal [NDC submissions](https://unfccc.int/NDCREG), and Annex 1 parties' [National Inventory Reports](https://unfccc.int/ghg-inventories-annex-i-parties/2025) (NIRs). A compilation of this inventory data is available [here](https://docs.google.com/spreadsheets/d/1F9opy0QXEVw5lcrjhuymbmWczSiNsauY/edit?usp=sharing&ouid=107483244733420618943&rtpof=true&sd=true).

[PRIMAP-hist](https://zenodo.org/records/17090760) estimates of total emissions with and without [LULUCF](https://unfccc.int/topics/land-use/workstreams/land-use--land-use-change-and-forestry-lulucf) are used to fill gaps in the official national inventory data. Internal gaps in the inventory data are filled to mimic year-on-year variation in the ‘HISTCR’ series of the PRIMAP data. Interpolated estimates are rescaled to be consistent with official data in the years before and after those gaps. Estimates are also extrapolated for the earliest and most recent years where official data are missing. If PRIMAP-hist and national inventory data are both available for a country for at least some years, the PRIMAP-hist data are only used to fill additional years in the inventory data if there is generally good agreement between the two in years where the data overlap (i.e., within a +/- 15% difference on average and never more than 25%). The PRIMAP-hist data are used directly for countries with no inventory data reported for years after 2010, except where a country has reported new estimates in its new NDC for total emissions in years before 2010 (e.g., in constructing a baseline for a future target). To ensure consistency, the same data source is always used for total emissions with and without LULUCF for a given country and year.
            """),
...
3. From a party’s 2030 NDC
            ...
            html.Br(),
            dcc.Markdown("""
Formulation (2) is only applied if a party has not already peaked its emissions **and** has not set a 2030 NDC target. Satisfying (2) represents a high bar for parties that have not yet peaked their emissions and should be seen as a sufficient but not necessary condition for credibility. Meeting this threshold implies that a party with a future peak will need to reduce its emissions on a steeper trajectory than a straight line from peak to net zero to achieve its 2035 NDC target (e.g., Thailand). A party’s 2035 target is classified as credible if any part of its target range is on a linear-or-steeper trajectory. In applying formulation (3), however, the lower end of the party’s 2035 target range is only judged against the lower end of its 2030 target, and the upper end of its 2035 target is only compared against the upper end of its 2030 target.
...
Downscaled [NGFS](https://www.ngfs.net/ngfs-scenarios-portal/) climate scenario data are obtained from the [NGFS Phase 5 Scenario Explorer](https://data.ene.iiasa.ac.at/ngfs/#/license) maintained by IIASA. The NGFS data include results for more individual countries/parties but from fewer models (GCAM, REMIND-MAgPIE, and MESSAGEix-GLOBIOM) compared with the IPCC data. Because of this, ranges are shown for the minimum and maximum 2035 emissions levels rather than the 5th to 95th percentiles.
            ...
            dcc.Markdown("""
Data and visualizations are provided under a [Creative Commons CC BY 4.0](https://creativecommons.org/licenses/by/4.0/) license, which allows unrestricted reuse with proper attribution. The dashboard may be cited as: **Chris Sall, “NDC Benchmarker” (2026), Cypress Climate Advisory, https://ndc-benchmarker.plotly.app**.
